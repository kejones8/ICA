...
state<-range(nodups_nas$st_burn_cnt)
trib<-range(nodups_nas$trib_burn_cnt)
cnty<-range(nodups_nas$cnty_burn_cnt)
cenpl<-range(nodups_nas$cenpl_burn_cnt)


which_high_fedfires<-nodups_nas[nodups_nas$fed_burn_cnt==range(nodups_nas$fed_burn_cnt)[2],]
which_high_stfires<-nodups_nas[nodups_nas$st_burn_cnt==range(nodups_nas$st_burn_cnt)[2],]
which_high_tribfires<-nodups_nas[nodups_nas$trib_burn_cnt==range(nodups_nas$trib_burn_cnt)[2],]
which_high_cntyfires<-nodups_nas[nodups_nas$cnty_burn_cnt==range(nodups_nas$cnty_burn_cnt)[2],]
which_high_cenplfires<-nodups_nas[nodups_nas$cenpl_burn_cnt==range(nodups_nas$cenpl_burn_cnt)[2],]
