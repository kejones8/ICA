...
#1. shapefile of surfman intersecting all burned areas
#2  shapefile of surfman intersecting all threatened areas
...
#6. a csv list of threatened jursidicitons by incident id (excluding those already burned)
...
surfman_buf<-read_sf("data\\JCA\\surfman_zerobuf.shp")
...
burn_inter_trimmed<-whatitdo_burn[,c("incident_id","Event_ID","START_YEAR","JrsdcUK","JrsdcUA","JrsdcUN","JrsdcUI","LndwnrK","LndwnrC","burn_inter","threat_inter")]
...
whatitdo_threat<-merge(threat_intersected,mtbs_withincid,by.x="Event_ID","mtbs_ids")
...
st_drop_geometry(threat_inter_trimmed)
...
write.csv(burn_incid_count_stcnty,"data\\JCA\\burn_incid_count_cntyst.csv")
write.csv(threat_incid_count_stcnty,"data\\JCA\\threat_incid_count_cntyst.csv")
